...
function useDigitalPersona() {
    ...
    useEffect(() => {
        ...
        const tryInit = () => {
            ...
            if (!sdk?.WebApi) { pollTimer = setTimeout(tryInit, 300); return; }

            let client;
            try { client = new sdk.WebApi(); } catch {
                setDeviceStatus("sdk_missing"); return;
            }
            clientRef.current = client;

            client.onDeviceConnected = (e) => {
                deviceRef.current = e?.deviceUid ?? e?.deviceID ?? e?.uid ?? "auto";
            ...
            client.onCommunicationFailed = (e) => {
                console.warn("[DP] Communication failed — is the Lite Client running?", e);
                setDeviceStatus("disconnected");
            };

            // Probe for already-connected device instead of waiting for the event
    ...
    const capture = useCallback(() => new Promise((resolve, reject) => {
        ...
        if (!sdk || !client) return reject(new Error("Scanner not ready."));

        let qualityScore = 80;
        client.onQualityReported  = (e) => { qualityScore = e.quality === 0 ? 90 : Math.max(10, 80 - e.quality * 10); };
        client.onSamplesAcquired  = (e) => {
            try {
                ...
                client.stopAcquisition().catch(() => {});
                resolve({
                    template : base64Data,          // PNG for display
                    fmd      : e.samples,           // raw samples string — send this for matching
                    quality  : qualityScore,
                    bitmap   : base64Data,
                    device   : "DigitalPersona U.are.U 4500",
                });
            } catch { reject(new Error("Failed to parse sample.")); }
        };
        client.onErrorOccurred = (e) => reject(new Error(`Scanner error: ${e?.error ?? "unknown"}`));

        const SampleFormat = sdk.SampleFormat ?? sdk.Formats?.SampleFormat;
        client.startAcquisition(SampleFormat.PngImage).catch(err => reject(new Error(err?.message ?? "Acquisition failed.")));
    }), []);
...
function FingerprintIcon({ className }) {
    return (
        <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round">
            <path d="M12 2C6.477 2 2 6.477 2 12" /><path d="M12 6c3.314 0 6 2.686 6 6" />
            <path d="M12 10a2 2 0 0 1 2 2c0 3-3 6-3 6" /><path d="M12 10a2 2 0 0 0-2 2c0 3 2 5 2 8" />
            <path d="M8 12a4 4 0 0 1 8 0" /><path d="M5 12a7 7 0 0 1 14 0" /><path d="M2 12a10 10 0 0 1 20 0" />
        </svg>
    );
}
...
function DeviceBadge({ status }) {
    const map = {
        ready:       { label: "🟢 Scanner Ready",      cls: "bg-green-50 dark:bg-green-900/20 text-green-600 border-green-200" },
        connecting:  { label: "🟡 Connecting…",         cls: "bg-amber-50  dark:bg-amber-900/20  text-amber-600  border-amber-200" },
        disconnected:{ label: "🔴 Disconnected",        cls: "bg-red-50   dark:bg-red-900/20   text-red-600   border-red-200" },
        sdk_missing: { label: "⚠ SDK not loaded",      cls: "bg-red-50   dark:bg-red-900/20   text-red-600   border-red-200" },
    };
    const { label, cls } = map[status] ?? map.connecting;
    return <span className={`text-[9px] font-semibold px-2 py-1 rounded-full border ${cls}`}>{label}</span>;
}
...
function ResultCard({ result, onReset }) {
    ...
    const logLabel    = LOG_TYPES.find(l => l.key === result.logType)?.label ?? result.logType;

    return (
        <div className={`rounded-xl border-2 p-5 flex flex-col items-center gap-3 text-center transition-all
            ${isMatch     ? "border-green-400 bg-green-50 dark:bg-green-900/20"
            : isDuplicate ? "border-amber-400  bg-amber-50  dark:bg-amber-900/20"
        ...
        >
            ...
            <p className={`text-sm font-bold
                ...
                {isMatch ? "Match Found" : isDuplicate ? "Already Logged" : "No Match"}
            </p>

            {/* Employee info */}
            {result.employee && (
                <div className="flex flex-col gap-0.5">
                    <p className="text-base font-semibold text-zinc-800 dark:text-zinc-100">{result.employee.name}</p>
                    <p className="text-[10px] text-zinc-500 dark:text-zinc-400">{result.employee.employid} · {result.employee.department}</p>
                    {result.employee.finger && (
                        <p className="text-[9px] text-zinc-400 dark:text-zinc-500 mt-0.5">Matched: {result.employee.finger}</p>
                    )}
                </div>
            )}
            ...
            {isMatch && result.log && (
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] text-zinc-500 dark:text-zinc-400">{result.log.logged_at}</span>
                </div>
            )}
            ...
            {!isMatch && (
                <p className="text-[10px] text-zinc-500 dark:text-zinc-400 italic">{result.message}</p>
            )}
...
function RecentLog({ log }) {
    const label = LOG_TYPES.find(l => l.key === log.log_type)?.label ?? log.log_type;
    return (
        <div className="flex items-center justify-between px-3 py-2 rounded-lg bg-zinc-50 dark:bg-zinc-800 border border-zinc-100 dark:border-zinc-700">
            <div className="min-w-0">
                <p className="text-[10px] font-semibold text-zinc-700 dark:text-zinc-200 truncate">{log.employee_name}</p>
                <p className="text-[8px] text-zinc-400 dark:text-zinc-500 truncate">{log.department}</p>
            </div>
...
export default function ScanLog() {
    ...
    const handleScan = useCallback(async () => {
        if (deviceStatus !== "ready") {
            setErrorMsg("Scanner not ready. Check the DigitalPersona Lite Client.");
            return;
        }
        setErrorMsg("");
        setResult(null);
        ...
        try {
            ...
            setErrorMsg(err.response?.data?.message ?? err.message ?? "Scan failed.");
            setScanState("idle");
        }
    ...
    const selectedColor = LOG_TYPES.find(l => l.key === selectedLogType)?.color ?? "green";
        ...
        <AuthenticatedLayout>
            ...
            <div className="h-full flex flex-col overflow-hidden p-4 gap-4">
                ...
                <div className="flex items-center justify-between flex-shrink-0">
                    <div>
                        <h1 className="text-sm font-semibold text-zinc-800 dark:text-zinc-100">Fingerprint Attendance</h1>
                ...
                <div className="flex-1 grid grid-cols-[1fr_300px] gap-4 min-h-0 overflow-hidden">
                    ...
                    <div className="flex flex-col gap-4 overflow-auto">
                        ...
                        <div className="rounded-xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-4">
                            ...
                            <div className="grid grid-cols-4 gap-2">
                                {LOG_TYPES.map(lt => {
                                    const isSelected = selectedLogType === lt.key;
                                    return (
                                        <button
                                            key={lt.key}
                                            onClick={() => { setSelectedLogType(lt.key); handleReset(); }}
                                            className={`px-2 py-2 rounded-lg border text-[10px] font-semibold transition-all duration-150 active:scale-95
                                                ${isSelected
                                                    ? LOG_COLOR_SELECTED[lt.color]
                        ...
                        <div className="rounded-xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-6 flex flex-col items-center gap-5">
                            ...
                            {/* Scanner window */}
                            <div className={`relative w-40 h-48 rounded-2xl border-2 overflow-hidden flex items-center justify-center transition-all duration-300
                                ${scanState === "scanning"
                            ...
                            >
                                {scanState === "scanning" && (
                                    <div className="absolute inset-0 overflow-hidden">
                                        <div className="absolute left-0 right-0 h-0.5 bg-blue-400 shadow-[0_0_8px_2px_rgba(96,165,250,0.6)]"
                                ...
                                <FingerprintIcon className={`w-20 h-20 transition-all duration-300
                                    ${scanState === "scanning"  ? "text-blue-300   dark:text-blue-600 animate-pulse"
                                    : scanState === "verifying" ? "text-violet-200 dark:text-violet-700 opacity-30"
                                    : scanState === "done" && result?.matched ? "text-green-400 dark:text-green-500"
                                    : scanState === "done"      ? "text-red-300   dark:text-red-600"
                            ...
                            <p className={`text-[11px] font-medium text-center
                                ...
                                : scanState === "verifying" ? "text-violet-600 dark:text-violet-400"
                                : scanState === "done" && result?.matched ? "text-green-600 dark:text-green-400"
                                : scanState === "done"      ? "text-red-600    dark:text-red-400"
                                :                             "text-zinc-400   dark:text-zinc-500"}`}
                            >
                                {scanState === "idle"      && "Ready to scan"}
                                {scanState === "scanning"  && "Scanning… hold still"}
                                {scanState === "verifying" && "Verifying fingerprint…"}
                                {scanState === "done"      && (result?.matched ? "Attendance recorded" : "Scan failed")}
                            </p>
                    ...
                    <div className="flex flex-col rounded-xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 overflow-hidden">
                        ...
                        <div className="flex-1 overflow-auto p-3 flex flex-col gap-1.5">
                            {recentLogs.length === 0 ? (
                                <p className="text-[10px] text-zinc-400 dark:text-zinc-500 italic text-center mt-4">No logs yet today.</p>
                            ) : (
                                recentLogs.map(log => <RecentLog key={log.id} log={log} />)
                            )}
                        </div>
